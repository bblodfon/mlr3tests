...
See more details about `BART`'s parameters on the online documentation [TODO].
...
What the prediction object actually stores internally is a 3d survival array:
```{r}
dim(p$data$distr)
```
...
An example: get all the posterior probabilities of the 3rd patient in the test set, at 12 months (1 year):
```{r}
p$data$distr[3, 12, ]
```
...
2. Right-corrected Log Loss (requires a survival distribution prediction - `distr`)
3. Harrell's C-index (requires a continuous ranking score prediction - `crank`)
```{r}
measures = mlr3::msrs(c("surv.graf", "surv.rcll", "surv.cindex"))
