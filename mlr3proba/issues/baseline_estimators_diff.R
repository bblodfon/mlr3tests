...
#' `gbm::basehaz.gbm` => computes the Breslow estimator of the baseline hazard function for a proportional hazard regression model.
...
library(survival)
df = t$data()
lines(exp(-survival::basehaz(coxph(t$formula(), df)))[, 1], col = 3) # CoxPH
